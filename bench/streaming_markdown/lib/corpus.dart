/// Deterministic synthetic corpus for the streaming-render benchmark.
///
...
String buildDocument({int targetChars = 12000, int seed = 42}) {
  ...
  while (out.length < targetChars) {
    ...
    final section = _sections[(i + rng.next(_sections.length)) % _sections.length];
...
String buildPathologicalDocument({int lines = 400}) {
  ...
  for (var i = 0; i < lines; i++) {
    out.writeln('  final field$i = compute(input$i, options: const Options('
...
String buildWideTableDocument({int rows = 300}) {
  ...
  for (var i = 0; i < rows; i++) {
    out.writeln('| $i | service-$i | ${i.isEven ? "ok" : "degraded"} '
